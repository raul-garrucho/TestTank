	...
	public class HotspotEditor : Editor
	{
		...
		public override void OnInspectorGUI ()
		{
			...
			else
			{
				...
				_target.interactionPosition = (Transform)CustomGUILayout.ObjectField<Transform>("Interaction Position:" ,_target.interactionPosition, true);

				if (_target.interactionPosition == null)
				{
					if (GUILayout.Button("Create", autoWidth))
					{
						string prefabName = "InteractionPosition";
						GameObject go = new GameObject();
						go.name = prefabName;                        
                        go.transform.position = _target.transform.position;
						_target.interactionPosition = go.transform;
						go.transform.parent = _target.transform;
					}
				}
