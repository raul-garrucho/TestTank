...
namespace AC
{
	...
	public class AnimEngine_Mecanim : AnimEngine
	{
		...
		public void ActionCharBodyExpressionGUI(ActionCharBodyExpression action, List<ActionParameter> parameters = null)
		{

#if UNITY_EDITOR
            action.bodyExpressionValueParameterId = Action.ChooseParameterGUI("Set Expression:", parameters, action.bodyExpressionValueParameterId, ParameterType.Integer);

            if (action.bodyExpressionValueParameterId < 0)
            {
		...
		public void ActionCharFaceExpressionGUI(ActionCharBodyExpression action, List<ActionParameter> parameters = null)
		{

#if UNITY_EDITOR
			action.faceExpressionValueParameterId = Action.ChooseParameterGUI("Set Expression:", parameters, action.faceExpressionValueParameterId, ParameterType.Integer);

			if (action.faceExpressionValueParameterId < 0)
			{
		...
		public void ActionCharExpressionAssignValues(ActionCharBodyExpression action, List<ActionParameter> parameters)
		{			
		...
		public float ActionCharExpressionRun(ActionCharBodyExpression action)
		{
			return ActionCharExpressionProcess(action);
		}
		...
		public void ActionCharBodyExpressionSkip(ActionCharBodyExpression action)
		{
			ActionCharExpressionProcess(action);
		}
		...
		protected float ActionCharExpressionProcess(ActionCharBodyExpression action)
		{		
			if (character.GetAnimator() == null)
			{
				return 0f;
			...
			if (!string.IsNullOrEmpty(action.bodyParameterName))
			{						
				
			}

			if (!string.IsNullOrEmpty(action.faceParameterName))
			{
		...
		public bool RequiresRememberAnimator(ActionCharBodyExpression action)
		{
			if (action.methodMecanim == AnimMethodCharMecanim.ChangeParameterValue ||
				action.methodMecanim == AnimMethodCharMecanim.PlayCustom)
			{
