...
namespace AC
{
	...
	[System.Serializable]
	public class ActionSpeechDecision : ActionCheck, ITranslatable
	{
		...
		public override string Title { get { return "Play Speech Decision"; }}
			...
			if (KickStarter.dialog && KickStarter.stateHandler)
			{
				if (KickStarter.speechManager.referenceSpeechFiles == ReferenceSpeechFiles.ByAddressable && lineID >= 0)
				{
					...
					if (!isRunning && !(isAwaitingAddressableAudio || isAwaitingAddressableLipsync))
					{
						...
						AsyncOperationHandle<IList<IResourceLocation>> handle = Addressables.LoadResourceLocationsAsync(filename);
