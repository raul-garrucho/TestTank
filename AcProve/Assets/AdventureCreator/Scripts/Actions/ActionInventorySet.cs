...
namespace AC
{
	...
	{
		private const string OUTFIT_PART_KEY = "UI_item-notification";
		private const string CONGRATULATIONS_KEY = "UI_notification-congratulations";
		...
		public bool executePlayerAnimation = false;

		private bool executingAnimation = false;
		...
		{
			if (KickStarter.runtimeInventory && !isRunning)
			{
				if (!setAmount)
				{
					amount = 1;
            ...
            if (executingAnimation || popUpShowing)
            {
				isRunning = true;
				return 1f;
            }
		...
		private async void AddItem (int _playerID, int _amount)
		{
            if (executePlayerAnimation)
            {
				executingAnimation = true;
				KickStarter.player.GetAnimator().SetBool("Actions", true);
				KickStarter.player.GetAnimator().SetTrigger("Pick");
				
				KickStarter.player.GetAnimator().SetBool("Actions", false);
			...
			else
			{
				if (addToFront)
				{
					invInstance = KickStarter.runtimeInventory.PlayerInvCollection.Insert (new InvInstance (invID, _amount), 0);
				}
				else
				{
					invInstance = KickStarter.runtimeInventory.PlayerInvCollection.Add (new InvInstance (invID, _amount));
                }
            }
		...
		private async void RemoveItem(int _playerID)
        {
			if (executePlayerAnimation)
			{
				executingAnimation = true;
				KickStarter.player.GetAnimator().SetBool("Actions", true);
				KickStarter.player.GetAnimator().SetTrigger("Give");
		...
		public override void ShowGUI (List<ActionParameter> parameters)
		{
			...
			if (inventoryManager != null)
			{
				...
				if (inventoryManager.items.Count > 0)
				{
					...
					executePlayerAnimation = EditorGUILayout.Toggle("Execute Player Animation: ", executePlayerAnimation);
