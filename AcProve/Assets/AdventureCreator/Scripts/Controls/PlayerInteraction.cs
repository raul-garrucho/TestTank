...
namespace AC
{
	...
	public class PlayerInteraction : MonoBehaviour
	{
			...
			if (KickStarter.player)
			{
				...
				else
				{
					...
					else
					{
                        ...
                        {
							decelerateCoroutine = StartCoroutine(DecelerateIEnu());
						}
		...
		IEnumerator DecelerateIEnu()
        {

			KickStarter.player.charState = CharState.Decelerate;

			Animator playerAnimator = KickStarter.player.GetAnimator();
			while (playerAnimator != null && playerAnimator.GetFloat(KickStarter.player.moveSpeedParameter) > 0.0001f )
            {
				KickStarter.player.ForceUpdateAnimator();
				yield return null;
            }
