	...
	public class Speech
	{
		...
		public Speech (Char _speaker, string _message, int lineID, bool _isBackground, bool _noAnimation, bool _preventSkipping = false, AudioClip audioOverride = null, TextAsset lipsyncOverride = null)
		{
			...
			if(_speaker)
            {
				if (coroutine != null)
				{
					_speaker.StopCoroutine(coroutine);
					coroutine = null;
				}
				coroutine = _speaker.StartCoroutine(DelayedUpdateDisplay());
			}
		...
		IEnumerator DelayedUpdateDisplay()
        {
			yield return null;
		    if( this != null)
            {
				KickStarter.eventManager.Call_OnUpdateDisplay(this, log.fullText);
			}
				...
				if (IsScrolling ())
				{
					if (!pauseGap)
					{
						...
						if (newCharIndex == 0 || newCharIndex != currentCharIndex)
						{
							...
							//Debug.Log("Number:" + currentCharIndex);
							KickStarter.eventManager.Call_OnUpdateDisplay(this, log.fullText);
						}
		...
		public Color GetPortraitRingColor()
		{
			if (speaker)
			{
				return speaker.GetPotraitRingColor();
			}
			return Color.white;
		}
		...
		public void EndMessage (bool forceOff = false)
		{
			if (holdForever)
			{
				if (continueState == ContinueState.None)
